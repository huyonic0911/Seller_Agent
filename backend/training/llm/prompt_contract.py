...
Import lại builder từ `core/prompt.py` (nguồn sự thật duy nhất) + tiện ích dựng
...
# Cho phép `from core...`/`from modules...` khi chạy `python -m training.llm.*` từ backend/.
_BACKEND = Path(__file__).resolve().parents[2]
if str(_BACKEND) not in sys.path:
    sys.path.insert(0, str(_BACKEND))

from core.config import DEFAULT_PERSONA  # noqa: E402
from core.prompt import build_system_text, build_user_text  # noqa: E402
from modules.llm.rag import ProductStore  # noqa: E402
